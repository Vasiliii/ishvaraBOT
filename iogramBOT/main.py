    ...
    if message.chat.type == ChatType.SUPERGROUP:
        if not db.check_warning(message.from_user.id):
            if not db.nights():  
                    ...
                    if checkSubChanel(await bot.get_chat_member(chat_id=config.chanel_id, user_id=message.from_user.id)) and message.from_user.id != config.bot_ignore_message:
                        ...
                        for spam in config.spam:
                            if spam in lower_message.lower():
                                    if not db.check_warning(message.from_user.id):
                                        ...
                                        message1 = await message.answer("Публикации на данную тему только на платной основе. Обретитесь к администратору группы: @NikitaTeleger")
                                        scheduler.add_job(delete, trigger='date', run_date = datetime.now() + timedelta(seconds=180), kwargs={'chat_id': message1.chat.id , 'message_id': message1.message_id})
                        ...
                        for keyword in config.estate:
                            if keyword in lower_message.lower() and checkAdmin(message.from_user.id) != True:
                                    ...
                                    scheduler.add_job(delete, trigger='date', run_date = datetime.now() + timedelta(seconds=180), kwargs={'chat_id': message2.chat.id , 'message_id': message2.message_id})
                                    await message.delete()  
                                    break
                        for bad_word in config.swears:
                            if bad_word in lower_message:
                                ...
                                scheduler.add_job(delete, trigger='date', run_date = datetime.now() + timedelta(seconds=180), kwargs={'chat_id': message3.chat.id , 'message_id': message3.message_id})
                        ...
                        scheduler.add_job(delete, trigger='date', run_date = datetime.now() + timedelta(seconds=180), kwargs={'chat_id': message4.chat.id , 'message_id': message4.message_id})
                        await message.delete()
                else:
                    await message.forward(config.delGroup_id)
                    await message.delete()
                ...
                scheduler.add_job(delete, trigger='date', run_date = datetime.now() + timedelta(seconds=180), kwargs={'chat_id': message5.chat.id , 'message_id': message5.message_id})
                await message.delete()     
        else:
            await message.bot.ban_chat_member(message.chat.id, message.from_user.id)
            await message.delete()
                ...
                if not db.mute(message.from_user.id):
                    if checkSubChanel(await bot.get_chat_member(chat_id=config.chanel_id, user_id=message.from_user.id)) and message.from_user.id != config.bot_ignore_message:
                        ...
                        for keyword in config.estate:
                            if keyword in lower_message.lower() and checkAdmin(message.from_user.id) != True:
                                    message2 = await message.answer(f'{message.from_user.first_name}, Вы можете разместить объявление по недвижимости только через публикацию объявления в нашем боте "МойДом"',reply_markup=nav.botMenu)
                                    scheduler.add_job(delete, trigger='date', run_date = datetime.now() + timedelta(seconds=180), kkwargs={'chat_id': message2.chat.id , 'message_id': message2.message_id})
                                    await message.delete() 
                                    break
                        for bad_word in config.swears:
                            if bad_word in lower_message:
